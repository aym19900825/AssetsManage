...
<div>
	<div class="text item">
		...
		<el-form :model="inspectionRepTem2Form" ref="inspectionRepTem2Form">
		  <el-table :data="inspectionRepTem2Form.inspectionList.filter(data => !search || data.DECRIPTION.toLowerCase().includes(search.toLowerCase()))" row-key="ID" border stripe height="280" highlight-current-row="highlight-current-row" style="width: 100%;" :default-sort="{prop:'inspectionRepTem2Form.inspectionList', order: 'descending'}" v-loadmore="loadMore">
	...
	<!-- 检验/检测报告模板 Begin -->
		<el-dialog :modal-append-to-body="false" title="选择基础数据——检验/检测报告模板" height="300px" :visible.sync="dialogVisible3" width="80%" :before-close="handleClose">
			<!-- 第二层弹出的表格 Begin-->
