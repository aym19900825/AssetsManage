	...
	<el-card class="box-card" :body-style="{ padding: '10px' }">
		...
		<div class="text item">
			...
			<el-form :model="product2Form" ref="product2Form">
			  <el-table :data="product2Form.inspectionList.filter(data => !search || data.PRO_NAME.toLowerCase().includes(search.toLowerCase()))" row-key="ID" border stripe height="400" highlight-current-row="highlight-current-row" style="width: 100%;" @cell-click="iconOperation" :default-sort="{prop:'product2Form.inspectionList', order: 'descending'}" v-loadmore="loadMore">
...
<script>
	export default {
		...
		methods: {
			...
			requestData_product2(index) {//加载数据
				var data = {
					page: this.page.currentPage,
					limit: this.page.pageSize,
				}
				var url = '/api/api-apps/app/product2';
				this.$axios.get(url, {
					params: data
				}).then((res) => {
					...
					this.product2Form.inspectionList = newarr;
				}).catch((wrong) => {})
			},
			...
			addfield_product2() { //插入行到产品类型Table中
			...
			saveRow (row) {//Table-操作列中的保存行
				this.$refs['product2Form'].validate((valid) => {
		          ...
		          if (valid) {
					var url = '/api/api-apps/app/product2/saveOrUpdate';
					var submitData = {
						"ID":row.ID,
						"NUM": row.NUM,
			...
			deleteRow(row) {//Table-操作列中的删除行
				this.$confirm('确定删除此产品类型吗？', '提示', {
                	...
                	var url = '/api/api-apps/app/product2/' + row.ID;
