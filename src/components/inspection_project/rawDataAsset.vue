	...
	<el-card class="box-card" :body-style="{ padding: '10px' }" shadow="never">
		...
		<div class="text item">
			...
			<el-form :model="rawDataAssetForm" ref="rawDataAssetForm">
			  <el-table :data="rawDataAssetForm.inspectionList.filter(data => !search || data.DECRIPTION.toLowerCase().includes(search.toLowerCase()))" row-key="ID" border stripe height="410" highlight-current-row="highlight-current-row" style="width: 100%;" @cell-click="iconOperation" :default-sort="{prop:'rawDataAssetForm.inspectionList', order: 'descending'}" v-loadmore="loadMore">
	...
	export default {
		...
		methods: {
			...
			requestData_rawDataAsset(index) {//加载数据
				...
				var url = '/api/api-apps/app/rawDataAsset';
				this.$axios.get(url, {
					params: data
				}).then((res) => {
					this.page.totalCount = res.data.count;	
			...
			saveRow (row) {//Table-操作列中的保存行
				this.$refs['rawDataAssetForm'].validate((valid) => {
		          ...
		          if (valid) {
					var url = '/api/api-apps/app/rawDataAsset/saveOrUpdate';
					var submitData = {
						"ID":row.ID,
						"P_NUM": row.P_NUM,
			...
			deleteRow(row) {//Table-操作列中的删除行
				this.$confirm('确定删除此产品类型吗？', '提示', {
                	...
                	var url = '/api/api-apps/app/rawDataAsset/' + row.ID;
