<template>	
    <div id="app">
        <el-row>
            <el-col span="24">
                <el-table size="mini" :data="master_user.listdata" border style="width: 100%" highlight-current-row>
...
<script>
        ...
        var generateId = {
            _count: 1,
            get(){return ((+new listdata()) + "_" + (this._count++))}
        };
    //主要内容
    export default {
       name: 'customer_management',
		components: {
            ...
            methods: {
                ...
                readMasterUser() {
                    //根据实际情况，自己改下啊 
                    app.master_user.listdata.map(i => {
                ...
                pwdChange(row, index, cg) {
                    ...
                    //是否是取消操作
                    if (!cg) {
                        if (!app.master_user.sel.id) app.master_user.listdata.splice(index, 1);
                    ...
                    if (row.isSet) {
                        //项目是模拟请求操作  自己修改下
                        (function () {
                            let listdata = JSON.parse(JSON.stringify(app.master_user.sel));
                            for (let k in listdata) row[k] = listdata[k];
                            app.$message({
                                type: 'success',
                                message: "保存成功!"
                            });
