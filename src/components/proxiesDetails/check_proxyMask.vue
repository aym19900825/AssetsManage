    ...
    <div class="mask_divbg" v-if="show">
            ...
            <div class="text-center" v-show="viewtitle">
              <span v-if="this.dataInfo.STATE!=3" class="pr10">
                ...
                <el-button
                  ...
                  v-show="approval&&nodeState=='3'"
                >
                  <i class="icon-edit-3"></i> 确认接收
                </el-button>
              </span>
              <el-button type="primary" round plain size="mini" @click="flowmap">
                <i class="icon-git-pull-request"></i> 流程地图
              </el-button>
              <el-button type="primary" round plain size="mini" @click="flowhistory">
                <i class="icon-plan"></i> 流程历史
              </el-button>
              <el-button type="primary" round plain size="mini" @click="viewpepole">
                <i class="icon-user"></i> 当前责任人
              </el-button>
            </div>
                  ...
                  <el-row>
                    ...
                    <el-col :span="12">
                      <el-form-item label="样品名称" prop="ITEM_NAME" label-width="110px">
                        <el-input v-model="dataInfo.ITEM_NAME" :disabled="special1"></el-input>
                      </el-form-item>
                    </el-col>
                  </el-row>
                  <el-row>
                    ...
                    <el-col :span="5">
                      <el-form-item label="数量" prop="ITEM_QUALITY" label-width="110px">
                        <el-input v-model="dataInfo.ITEM_QUALITY" :disabled="special1"></el-input>
                      </el-form-item>
                    </el-col>
                  </el-row>
                  <el-row>
                ...
                <div class="el-collapse-item pt10 pr20 pb20" aria-expanded="true" accordion>
                  <el-tabs v-model="activeName" @tab-click="handleClick">
                    <el-tab-pane label="检测依据" name="first">
                      ...
                      <el-table
                        :data="dataInfo.INSPECT_PROXY_BASISList"
                        row-key="ID"
                        border
                        stripe
                        :fit="true"
                        ...
                        :default-sort="{prop:'dataInfo.INSPECT_PROXY_BASISList', order: 'descending'}"
                      >
                        <el-table-column prop="S_NUM" label="编码" sortable width="150px">
                          <template slot-scope="scope">
                            <el-form-item
                              :prop="'INSPECT_PROXY_BASISList.'+scope.$index + '.S_NUM'"
                              :rules="[{required: true, message: '请输入', trigger: 'blur'}]"
                            >
                              <el-input
                                v-if="scope.row.isEditing"
                                size="small"
                                v-model="scope.row.S_NUM"
                                placeholder="请输入"
                              >
                        ...
                        <el-table-column prop="SS_NUM" label="标准编号" sortable width="150px">
                          <template slot-scope="scope">
                            <el-form-item
                              :prop="'INSPECT_PROXY_BASISList.'+scope.$index + '.SS_NUM'"
                              :rules="[{required: true, message: '请输入', trigger: 'blur'}]"
                            >
                              <el-input
                                v-if="scope.row.isEditing"
                                size="small"
                                v-model="scope.row.SS_NUM"
                                placeholder="请输入"
                              >
                                <el-button slot="append" icon="el-icon-search"></el-button>
                              </el-input>
                        ...
                        <el-table-column prop="VERSION" label="版本" sortable width="120px">
                          <template slot-scope="scope">
                            <el-form-item
                              :prop="'INSPECT_PROXY_BASISList.'+scope.$index + '.VERSION'"
                              :rules="[{required: true, message: '请输入', trigger: 'blur'}]"
                            >
                              <el-input
                                v-if="scope.row.isEditing"
                        ...
                        <el-table-column fixed="right" label="操作" width="120px">
                          <template slot-scope="scope">
                            <el-button
                              @click.native.prevent="deleteRow(scope.$index,scope.row,'basisList')"
                              type="text"
                              size="small"
                              v-if="!viewtitle"
                            >
                    ...
                    <el-tab-pane label="检测项目与要求" name="second">
                      ...
                      <el-table
                        :data="dataInfo.INSPECT_PROXY_PROJECList"
                        row-key="ID"
                        border
                        stripe
                        :fit="true"
                      ...
                      >
                        ...
                        <el-table-column prop="P_NUM" label="检测项目编号" sortable width="120px">
                          <template slot-scope="scope">
                            <el-form-item
                              :prop="'INSPECT_PROXY_PROJECList.'+scope.$index + '.P_NUM'"
                              :rules="[{required: true, message: '请输入', trigger: 'blur'}]"
                            >
                              <el-input
                                v-if="scope.row.isEditing"
                                size="small"
                                v-model="scope.row.P_NUM"
                                placeholder="请输入"
                              >
                                <el-button slot="append" icon="el-icon-search"></el-button>
                              </el-input>
                        ...
                        <el-table-column prop="P_DESC" label="检测项目描述" sortable>
                          <template slot-scope="scope">
                            <el-form-item
                              :prop="'INSPECT_PROXY_PROJECList.'+scope.$index + '.P_DESC'"
                              :rules="[{required: true, message: '请输入', trigger: 'blur'}]"
                            >
                              <el-input
                                v-if="scope.row.isEditing"
                                size="small"
                                v-model="scope.row.P_DESC"
                                placeholder="请输入"
                              ></el-input>
                        ...
                        <el-table-column prop="REMARKS" label="要求" sortable>
                          <template slot-scope="scope">
                            <el-form-item
                              :prop="'INSPECT_PROXY_PROJECList.'+scope.$index + '.REMARKS'"
                            >
                              <el-input
                                size="small"
                        ...
                        >
                          <template slot-scope="scope">
                            <el-form-item
                              :prop="'INSPECT_PROXY_PROJECList.'+scope.$index + '.REALITY_PRICE'"
                            >
                              <el-input
                                size="small"
                        ...
                        <el-table-column prop="VERSION" label="项目版本" sortable width="120px">
                          <template slot-scope="scope">
                            <el-form-item
                              :prop="'INSPECT_PROXY_PROJECList.'+scope.$index + '.VERSION'"
                              :rules="[{required: true, message: '请输入', trigger: 'blur'}]"
                            >
                              <el-input
                                v-if="scope.row.isEditing"
                    ...
                    <el-tab-pane label="分包要求" name="third">
                      ...
                      <el-table
                        :data="dataInfo.CHECK_PROXY_CONTRACTList"
                        row-key="ID"
                        border
                        stripe
                        :fit="true"
                      ...
                      >
                        <span v-show="!viewtitle">
                          <el-table-column prop="iconOperation" fixed label width="50px"></el-table-column>
                        </span>
                        ...
                        <el-table-column prop="VENDORDesc" label="分包方名称" sortable width="120px">
                          <template slot-scope="scope">
                            <el-form-item
                              :prop="'CHECK_PROXY_CONTRACTList.'+scope.$index + '.VENDORDesc'"
                              :rules="[{required: true, message: '请输入', trigger: 'blur'}]"
                            >
                              <el-input
                                v-if="scope.row.isEditing"
                        ...
                        <el-table-column prop="DEPTTYPEDesc" label="机构类型" sortable width="120px">
                          <template slot-scope="scope">
                            <el-input
                              ...
                              :disabled="true"
                              size="small"
                              v-model="scope.row.DEPTTYPEDesc"
                              placeholder
                            ></el-input>
                        ...
                        <el-table-column prop="PRODUCT_TYPE" label="产品类别" sortable width="120px">
                          <template slot-scope="scope">
                            <el-form-item
                              :prop="'CHECK_PROXY_CONTRACTList.'+scope.$index + '.P_DESC'"
                            >
                              <el-input
                                v-if="scope.row.isEditing"
                                :disabled="true"
                        ...
                        <el-table-column prop="BASIS" label="检测技术依据" sortable width="150px">
                          <template slot-scope="scope">
                            <!--委托书新建时事中心的不可输 2 -->
                            <el-input
                              v-show="!viewtitle&&scope.row.DEPTTYPE==1&&dataInfo.N_CODE==null"
                              size="small"
                              v-model="scope.row.BASIS"
                              placeholder="请输入"
                            ></el-input>
                            <el-input
                              v-show="!viewtitle&&scope.row.DEPTTYPE==2&&dataInfo.N_CODE==null"
                              size="small"
                              v-model="scope.row.BASIS"
                              placeholder="请输入"
                              :disabled="true"
                            ...
                            >
                              <el-button
                                slot="append"
                                icon="el-icon-search"
                                @click="basis1(scope.row,scope.$index)"
                              ></el-button>
                            </el-input>
                            <el-input
                              v-show="!viewtitle&&scope.row.isEditing&&scope.row.DEPTTYPE==2&&dataInfo.N_CODE!=null"
                              size="small"
                              v-model="scope.row.BASIS"
                              placeholder="请输入"
                              :disabled="true"
                            ></el-input>
                            <span v-show="viewtitle&&!scope.row.isEditing">{{scope.row.BASIS}}</span>
                          </template>
                        </el-table-column>
                        ...
                        <el-table-column prop="P_REMARKS" label="检测项目内容" sortable width="200px">
                          <template slot-scope="scope">
                            <el-form-item
                              :prop="'CHECK_PROXY_CONTRACTList.'+scope.$index + '.P_REMARKS'"
                            ...
                            >
                              ...
                              >
                                <el-button
                                  slot="append"
                                  icon="el-icon-search"
                                  @click="contents(scope.row,scope.$index)"
                                ></el-button>
                              </el-input>
                              <!-- 新建时事中心的不可输 2 -->
                              <el-input
                              ...
                              <span v-show="viewtitle&&!scope.row.isEditing">{{scope.row.P_REMARKS}}</span>
                            </el-form-item>
                          </template>
                        </el-table-column>
                        ...
                        <el-table-column prop="REQUIRES" label="对环境和操作人员要求" sortable width="220px">
                          <template slot-scope="scope">
                            <el-form-item
                              :prop="'CHECK_PROXY_CONTRACTList.'+scope.$index + '.REQUIRES'"
                            >
                              <el-input
                                v-if="scope.row.isEditing"
                                size="small"
                                v-model="scope.row.REQUIRES"
                                placeholder="请输入内容"
                              ></el-input>
                        ...
                        <el-table-column prop="Q_TYPE" label="对分包报告/证书的要求" sortable width="220px">
                          <template slot-scope="scope">
                            <el-form-item
                              :prop="'CHECK_PROXY_CONTRACTList.'+scope.$index + '.Q_TYPE'"
                              :rules="[{required: true, message: '请输入', trigger: 'blur'}]"
                            >
                              <el-input
                                v-if="scope.row.isEditing"
                        ...
                        <el-table-column prop="CHECKCOST" label="检测费用(元)" sortable width="160px">
                          <template slot-scope="scope">
                            <el-form-item
                              :prop="'CHECK_PROXY_CONTRACTList.'+scope.$index + '.CHECKCOST'"
                              :rules="[{required: true, message: '请输入数字', trigger: 'change'}]"
                            >
                              <el-input
                                v-if="scope.row.isEditing"
                                id="testprice"
                        ...
                        <el-table-column fixed="right" label="操作" width="120">
                          <template slot-scope="scope">
                            <el-button
                              @click.native.prevent="deleteRow(scope.$index,scope.row,'requestList')"
                              type="text"
                              size="small"
                              v-if="!viewtitle"
                            >
                  ...
                  <el-row>
                    <el-col :span="4">
                      <el-form-item label="交付委托方份数" prop="REPORT_QUALITY" label-width="120px">
                        <el-input
                          v-model="dataInfo.REPORT_QUALITY"
                          :disabled="noedit"
                          @blur="compare"
                        ></el-input>
                      </el-form-item>
                    ...
                    <el-col :span="8">
                      <el-form-item label="付款方式" prop="PAYMENT_METHOD" label-width="110px">
                        <el-select
                          v-model="dataInfo.PAYMENT_METHOD"
                          placeholder="请选择"
                          style="width: 100%;"
                          :disabled="noedit"
                        >
                  ...
                  <el-row>
                    ...
                    <el-col :span="8">
                      <el-form-item label="主检组" prop="MAINGROUP" label-width="110px">
                        <el-select
                          v-model="dataInfo.MAINGROUP"
                          filterable
                          allow-create
                          default-first-option
                          placeholder="请选择"
                          :disabled="noedit"
                          @change="getmaingroup($event)"
                          @visible-change="visablemaingroup($event)"
                          style="width: 100%;"
                        >
                          <el-option
                            v-for="(data,index) in maingroup"
                            :key="index"
                            :value="data.id"
                            :label="data.fullname"
                          ></el-option>
                    ...
                    <el-col :span="8">
                      <el-form-item label="主检负责人" prop="LEADER" label-width="110px">
                        <el-select
                          ...
                          default-first-option
                          placeholder="请选择"
                          @visible-change="visableleader($event)"
                          :disabled="noedit"
                        >
                          <el-option
                            v-for="(data,index) in leaderdata"
                            :key="index"
                            :value="data.id"
                            :label="data.nickname"
                          ></el-option>
      ...
      >
        <el-table
          ...
          @cell-click="iconOperation"
          :default-sort="{prop:'CUSTOMER_PERSONList', order: 'descending'}"
          v-loading="loading"
        ...
        >
          ...
          <el-table-column label="联系人" sortable width="150px" prop="PERSON"></el-table-column>
          <el-table-column prop="PHONE" label="联系电话" sortable width="150px"></el-table-column>
          <el-table-column prop="FAX" label="传真" sortable width="150px"></el-table-column>
      ...
      <teststandardmask
        ref="standardchild"
        @testbasis="addbasis"
        @testbasisnum="testbasisnum"
        @testbasisname="testbasisname"
        @testbasisprover="testbasisprover"
      ></teststandardmask>
      <!-- 检验项目  -->
...
export default {
        ...
        CHECK_COST: [
          {
            required: false,
            trigger: "blur",
            validator: this.Validators.isPrices
          }
        ], //合同收费(元)
        ACTUALCOST: [
          {
  ...
  methods: {
    ...
    compare() {
      if (
        parseInt(this.dataInfo.REPORT_QUALITY) >
        parseInt(this.dataInfo.REPORT_COUNT)
      ) {
      ...
      columns.forEach((column, index) => {
        if (index === 0) {
          ...
          if (!values.every(value => isNaN(value))) {
            ...
            this.$forceUpdate();
            this.dataInfo.CHECK_COST = this.number_format(
              parseFloat(paramData2.replace(/,/g, "").replace("元", "")) +
                parseFloat(paramData1.replace(/,/g, "").replace("元", "")),
              2
            );
          } else {
            sums[index] = " ";
    ...
    toFixedPrice(price) {
      ...
      if (!re.test(money) || money == "") {
        ...
        var num = parseFloat(this.toNum(money))
          .toFixed(2)
          .toString()
          .split(".");
    ...
    build() {
      ...
      var Url =
        this.basic_url +
        "/api-apps/app/inspectPro2/operate/createWorkorder?ID=" +
        dataid +
        "&fileUrl=" +
        Config.file_url;
      this.$axios
    ...
    visible() {
      this.$axios
        .get(this.basic_url + "/api-user/users/currentMap", {})
        .then(res => {
      ...
      this.$axios
        ...
        .then(res => {
          ...
          for (var m = 0; m < res.data.INSPECT_PROXY_PROJECList.length; m++) {
            ...
            res.data.INSPECT_PROXY_PROJECList[m].INSPECT_GROUP = Number(
              res.data.INSPECT_PROXY_PROJECList[m].INSPECT_GROUP
            );
          }
          // 分包要求
          for (var n = 0; n < res.data.CHECK_PROXY_CONTRACTList.length; n++) {
            ...
            if (!!res.data.CHECK_PROXY_CONTRACTList[n].CHECKCOST) {
              if (
                res.data.CHECK_PROXY_CONTRACTList[n].CHECKCOST.indexOf(",") ==
                -1
              ) {
                res.data.CHECK_PROXY_CONTRACTList[
                  n
                ].CHECKCOST = this.toFixedPrice(
                  res.data.CHECK_PROXY_CONTRACTList[n].CHECKCOST
                );
              }
            }
            res.data.CHECK_PROXY_CONTRACTList[n].INSPECT_GROUP = Number(
    ...
    refresh() {
      ...
      var url =
        this.basic_url +
        "/api-apps/app/inspectPro2/flow/Executors/" +
        this.dataid;
      this.$axios.get(url, {}).then(res => {
    ...
    detail(dataid) {
      ...
      this.$axios
        ...
        .then(res => {
          ...
          var date = new Date();
          this.dataInfo.CHANGEDATE = this.$moment(date).format(
            "YYYY-MM-DD HH:mm:ss"
          );
          this.docParm = {
            model: "edit",
    ...
    view(dataid) {
      ...
      this.$axios
        .get(
          this.basic_url +
            "/api-apps/app/inspectPro2/flow/NodeId/" +
            this.dataid,
          {}
        )
        .then(res => {
      ...
      //判断启动流程和审批的按钮是否显示
      var url =
        this.basic_url + "/api-apps/app/inspectPro2/flow/isStart/" + dataid;
      this.$axios.get(url, {}).then(res => {
        if (res.data.resp_code == 1) {
          this.start = true;
          this.approval = false;
    ...
    basisleadbtn2(val) {
      ...
      if (val == "maintable") {
        if (this.dataInfo.INSPECT_PROXY_BASISList.length == 0) {
          ...
          for (
            var i = 0;
            i < this.dataInfo.INSPECT_PROXY_BASISList.length;
            i++
          ) {
    ...
    addproject(value) {
      for (var i = 0; i < value.length; i++) {
        ...
        var url =
          this.basic_url +
          "/api-apps/appSelection/professionGro/list?P_NUM_wheres=" +
          value[0].P_NUM;
        this.$axios
          .get(url, {})
          .then(res => {
    ...
    visablemaingroup: function(callback) {
      //只有回调参数为false时才触发 ctx.getAreaListDataSearch(vc,1)这个函数;
      if (callback) {
        if (
          this.dataInfo.R_VENDOR == "" ||
          this.dataInfo.R_VENDOR == undefined
        ) {
          this.$message({
            message: "请先选择承检单位",
            type: "error"
    ...
    visableleader: function(callback) {
      if (callback) {
        if (
          this.dataInfo.MAINGROUP == "" ||
          this.dataInfo.MAINGROUP == undefined
        ) {
          this.$message({
            message: "请先选择主检组",
            type: "error"
    ...
    RVENDORSelect() {
      var url =
        this.basic_url +
        "/api-user/depts/findByPid/" +
        this.$store.state.currentcjdw[0].id;
      this.$axios
        .get(url, {})
        .then(res => {
    ...
    getmaingroup(maingroupid) {
      ...
      this.dataInfo.LEADER = "";
      var url =
        this.basic_url + "/api-user/users/usersByDept?deptId=" + maingroupid;
      this.$axios
        .get(url, {})
        .then(res => {
    ...
    requestData(val) {
      ...
      var url =
        this.basic_url +
        "/api-apps/app/customer/CUSTOMER/" +
        this.dataInfo.V_NAME;
      this.$axios
        .get(url, {
          params: data
        })
    ...
    startup() {
      this.$refs.dataInfo.validate(valid => {
        if (valid) {
          var url =
            this.basic_url + "/api-apps/app/inspectPro2/flow/" + this.dataid;
          this.$axios.get(url, {}).then(res => {
            if (res.data.resp_code == 1) {
              this.$message({
                message: res.data.resp_msg,
    ...
    approvals() {
      ...
      var url =
        this.basic_url + "/api-apps/app/inspectPro2/flow/isEnd/" + this.dataid;
      this.$axios.get(url, {}).then(res => {
        if (res.data.resp_code == 0) {
          this.$message({
            message: res.data.resp_msg,
          ...
          var url =
            this.basic_url +
            "/api-apps/app/inspectPro2/flow/isExecute/" +
            this.dataid;
          this.$axios.get(url, {}).then(res => {
            if (res.data.resp_code == 1) {
              this.$message({
              ...
              var url =
                this.basic_url +
                "/api-apps/app/inspectPro2/flow/customFlowValidate/" +
                this.dataid;
              this.$axios.get(url, {}).then(res => {
                if (res.data.resp_code == 1) {
                  this.$message({
    ...
    outbasis() {
      ...
      var url = this.basic_url + '/api-apps/appSelection/workNotCheb/page?N_CODE_wheres='+this.dataInfo.N_CODE+'&DEPTTYPE=1&S_NAME_where_not_in='+arr;
      this.$axios
        .get(url, {})
        .then(res => {
    ...
    outcontents() {
      var url =
        this.basic_url +
        "/api-apps/appSelection/workNotCheo/page?N_CODE_wheres=" +
        this.dataInfo.N_CODE +
        "&DEPTTYPE=1";
      this.$axios
        .get(url, {})
        .then(res => {
    ...
    determinebasic() {
      var url =
        this.basic_url +
        "/api-apps/app/inspectionSta2?PRO_NUM_wheres=" +
        this.dataInfo.PRO_NUM +
        "&NUM_wheres=" +
        this.dataInfo.P_NUM +
        "&S_NUM_where_not_in=" +
    ...
    weituo() {
      ...
      this.$axios
        ...
        .then(res => {
          let datas = res.data.data;
          this.weituodata = res.data.data;
          for (let i = 0; i < datas.length; i++) {
            this.weituoname.push(datas[i].customername);
          }
    ...
    handleSelect(item) {
      ...
      for (let i = 0; i < this.weituodata.length; i++) {
        if (item.value == this.weituodata[i].customername) {
          ...
          this.dataInfo.V_ADDRESS = this.weituodata[i].customeraddress; //地址
          this.dataInfo.V_ZIPCODE = this.weituodata[i].customerzipcode; //邮编
          this.dataInfo.VENDOR = this.weituodata[i].customercode; //委托方名称的信用代码
          this.dataInfo.P_NAME = this.weituodata[i].customerid;
          this.dataInfo.P_NAMEDesc = this.weituodata[i].customername; //生产单位
          this.dataInfo.PRODUCT_UNIT = this.weituodata[i].customercode; //生产单位的信用代码
          //接到值的时候在去请求一下姓名的接口
          this.requestData("default");
          if (this.weituodata[i].depttype == 1) {
            //中心内（中心外）
            this.dataInfo.PRODUCE_TYPE = 1;
            this.dataInfo.DEPUTE_TYPE = 1;
          } else {
          ...
          if (!!this.weituodata[i].itemid) {
            //样品有值的时候
            var url = this.basic_url + "/api-apps/app/item/" + itemid;
            this.$axios.get(url, {}).then(res => {
              this.dataInfo.P_NAME = res.data.P_NAME; //生产单位
              this.dataInfo.ITEM_NAME = res.data.DESCRIPTION; //样品名称
              this.dataInfo.ITEM_ID = res.data.ID; //样品id
              this.dataInfo.ITEMNUM = res.data.ITEMNUM; //样品数量
              this.dataInfo.ITEM_MODEL = res.data.MODEL; //模型
              this.dataInfo.ITEM_QUALITY = res.data.QUATITY; //质量
              this.dataInfo.PRODUCT = res.data.PRODUCT; //产品名称
              this.dataInfo.PRODUCT_TYPE = res.data.PRODUCT_TYPE; //产品类别
              this.dataInfo.PRO_NUM = res.data.PRO_NUM; //产品编号
              this.dataInfo.PRO_VERSION = res.data.P_VERSION; //产品版本
              this.dataInfo.P_NUM = res.data.P_NUM; //产品类别编号
              this.dataInfo.P_VERSION = res.data.P_VERSION; //产品类别版本
              this.dataInfo.P_NAME = res.data.P_NAME; //生产单位名称id
              this.dataInfo.P_NAMEDesc = res.data.P_NAMEDesc; //生产单位name
              this.special1 = true;
              this.special = true;
              this.special2 = true; //样品名称
              this.PNAME = true; //生产单位
    ...
    logo() {
      var url =
        this.basic_url +
        "/api-user/dicts/findChildsByCode?code=identification ";
      this.$axios
        .get(url, {})
        .then(res => {
          this.logos = res.data;
    ...
    ITEM_STATUS() {
      var url =
        this.basic_url +
        "/api-user/dicts/findChildsByCode?code=item_accept_status";
      this.$axios
        .get(url, {})
        .then(res => {
...
.table-func-middle button:hover {
  width: 100px;
}
/*.el-form-item__error {
	top: 18%;
